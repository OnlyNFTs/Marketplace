...
openUserInfo = async () => {
    ...
    if (user){    
        ...
        if (!profilePageCheck) {
            ...
            if (!await user.attributes.referrer) {
                document.getElementById("profileCreateButton").onclick = () => $('#enterReferrer').modal('show');
...
submitRefferal = async () => {
    if (userReferrerField.value.length == 24) {
        ...
        userReferrerFieldVal = userReferrerField.value;
        const pointer = userReferrerFieldVal;

        const userQuery = new Moralis.Query(Moralis.User);
        userQuery.equalTo("objectId", userReferrerField.value);
        const userObject = await userQuery.first({});
        user.set('referrer', userObject);
        await user.save();
        alert("Submited!");
    } else  {
